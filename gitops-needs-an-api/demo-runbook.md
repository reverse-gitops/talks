...
| 18:00–24:15 | The mechanism: cells, the watch/audit join, the commit window | idle |
| 24:15–36:00 | **Demo 2** — Git, conflict, and the one place the room works together | **in use** |
...
- **The rounds are now called `demo1` and `evaluation`, and those names are
  ...
  `chore(demo2): 1 change in evaluation from …` instead of carrying a date
  nobody needs. But it removes the escape hatch this runbook used to lean on.
...
- [ ] **Delete the sessions and let Flux re-seed them.** Pushing is not enough:
      ...
      The old dated sessions (`demo1-round-2026-09-15` and friends) are gone by
      the same command plus `prune`. Confirm nothing dated is left.
...
screen and let the bars fill. That is the warm-up, and it earns the room's
attention for what follows.
...
**Clock 24:15 → 36:00, eleven and three quarter minutes.** The room is still signed in from demo 1;
...
`trust`, `first`, `adopt` (0–10, renders as a live average to one decimal, and
it keeps moving while you talk) and `missing` (free text).
...
synthetic (`@demo.invalid`). Keep that property. Point people at
